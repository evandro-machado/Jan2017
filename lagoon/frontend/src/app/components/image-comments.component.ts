...
export class ImageComments {
	...
	constructor (private _userService: UserService, private _commentService: CommentService, private _photoService: PhotoService) {
	...
	onSubmit() {
		...
		this._commentService.addComment(this.newComment).subscribe(
			photo => this._photoService.getPhotoById(this.photo.photoId).subscribe(
